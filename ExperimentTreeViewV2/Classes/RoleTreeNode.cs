...
namespace ExperimentTreeViewV2.Classes
{
    ...
    public class RoleTreeNode:TreeNode, ISerializable
    {
        ...
        {
            // i gt parent
            if(ParentRoleTreeNode != null)
            {
                // disown myself
                ParentRoleTreeNode._children.Remove(this);
                ParentRoleTreeNode.Nodes.Remove(this);
                return true;
            }
            return false;
        }
        ...
        protected RoleTreeNode(SerializationInfo info, StreamingContext context)
        {
            ...
            this.isLeader = info.GetBoolean("isLeader");
        ...
        public void GetAllUniqueRoles(ref Dictionary<String, List<String>> foundNodes)
        {
            if (foundNodes.ContainsKey(_role.Name))
            {
                foundNodes[_role.Name].Add(_role.UUID);
            }
            else
            {
                List<String> uuids = new List<String>();
                uuids.Add(_role.UUID);
                foundNodes.Add(_role.Name, uuids);
            }
            ...
            if (this.ChildRoleTreeNodes.Count > 0)//Note: This if block may not be necessary at all. Though the logic works.
            {
                ...
                for (i = 0; i < this.ChildRoleTreeNodes.Count; i++)
                {
                    this.ChildRoleTreeNodes[i].GetAllUniqueRoles(ref foundNodes);
                }
            }
